...
The editor ships with 40+ built-in presets organized by category.
...
```js
import { PRESETS, getPreset, getPresetsByCategory, searchPresets } from '@bezier-curve-editor/core/presets'
...
All power-curve families follow the same `in` / `out` / `in-out` pattern with progressively more aggressive acceleration.
...
::: tip Overshoot mode
These presets use Y values outside `[0, 1]`. Enable `overshoot` attribute or set `bounds="free"` to use them fully.
:::
