...
Fired once when pointer is released (committed value). Same detail shape as `input`.
...
Fired when user activates the copy action. `detail.cssValue` contains the copied string.

## `invalid`

Fired when a value is rejected due to bounds validation. `detail.value` is the rejected input.
